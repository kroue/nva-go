...
const ADMIN_EMAIL = 'admin@nvago.com';
...
const AdminMessages = () => {
  ...
  const fetchChats = async () => {
    try {
      ...
      // Sort disputes first
      chatList.sort((a, b) => {
        if (a.isDispute && !b.isDispute) return -1;
        if (!a.isDispute && b.isDispute) return 1;
      ...
      console.error('Exception fetching admin chats:', error);
    }
  };
  ...
  // Auto-scroll when messages append or when switching chats
  useEffect(() => {
    scrollToBottom();
  ...
  const handleResolveDispute = () => {
    if (!selectedChat || !selectedChat.isDispute) return;

    if (window.confirm('Are you sure this dispute is resolved?')) {
      resolveDispute();
    }
  };
  ...
  const resolveDispute = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Error sending resolution message:', error);
      ...
      console.error('Exception resolving dispute:', error);
      alert('Failed to resolve dispute: ' + error.message);
    }
  };
  ...
  const disputeCount = chats.filter(c => c.isDispute).length;
      ...
      <div className="Messages-header">
        <div className="Messages-header-content">
          ...
          <div className="Messages-header-text">
            ...
            <p>Manage customer disputes and conversations</p>
          </div>
        </div>
        <div className="Messages-stats">
          ...
          <div className="stat-badge dispute">
            <span className="stat-number">{disputeCount}</span>
        ...
        <div className="Messages-chat-list">
          ...
          <div className="Messages-chat-list-scroll">
            {chats.map(chat => (
              <div
                key={chat.id}
                className={`Messages-chat-item ${selectedChat?.id === chat.id ? 'active' : ''} ${chat.isDispute ? 'dispute' : ''}`}
                onClick={() => handleChatSelect(chat)}
              >
                <div className="chat-avatar">
                  {chat.isDispute ? (
                    <WarningAmberOutlinedIcon style={{ color: '#FFA500' }} />
                  ) : (
            ...
            <>
              <div className="Messages-chat-header">
                <div className="chat-header-left">
                  <div className="chat-avatar-large">
                    {selectedChat.isDispute ? (
                      <WarningAmberOutlinedIcon style={{ fontSize: 28, color: '#FFA500' }} />
                    ) : (
                  ...
                  <div className="chat-header-info">
                    <h2>
                      {selectedChat.isDispute && '🚨 DISPUTE - '}
              ...
              <div className="Messages-messages-container" ref={listRef}>
                {messages.map((message, index) => {
                  ...
                  const isSent = message.sender === ADMIN_EMAIL;
